...
TARGET_DIR=$OUT
if [ -z "$MK_RELEASE" ] && [ -z "$MK_HISTORY" ]; then
    PREBUILT_DIR=$TOP/prebuilts/chromium/$DEVICE/deodex
    BUILD=deodex
else
    PREBUILT_DIR=$TOP/prebuilts/chromium/$DEVICE/odex
    BUILD=odex
fi
...
mkdir -p $PREBUILT_DIR/webview
mkdir -p $PREBUILT_DIR/lib

if [ -d $TARGET_DIR ]; then
    echo "Copying files..."
    cp -rd $TARGET_DIR/system/app/webview $PREBUILT_DIR/webview/
...
(cat << EOF) | sed s/__DEVICE__/$DEVICE/g | sed s/__BUILD__/$BUILD/g > $PREBUILT_DIR/chromium_prebuilt.mk
# Copyright (C) 2014 The MoKee OpenSource Project
#
# Licensed under the Apache License, Version 2.0 (the "License");
# you may not use this file except in compliance with the License.
...
LOCAL_PATH := prebuilts/chromium/__DEVICE__/__BUILD__
