...
read -e -p "Enter the path to your build source Directory: " -i "~/mkkk" FILEPATH
eval FILEPATH=$SAUCE
...
read -e -p "What folder would you like to use for ccache? Typicaly ~/.ccache: " -i "~/.ccache" CCSTORAGE
eval CCSTORAGE=$CCSTORAGE
	fi
...
read -r -p "Do you want to Copy to FTP Storage Folder?" response
if [[ $response =~ ^([yY][eE][sS]|[yY])$ ]]
then
    FTPSTR=y
else
...
# What folder would you like to use for ccache? Typicaly ~/.ccache
read -e -p "What folder would you like to use for FTP ROM Storage folder?" -i "//media/daavvis/storage/uploadfolder/full" FTPR
eval FTPR=$FTPR

# FTP upload OTA folder
read -e -p "What folder would you like to use for FTP OTA Storage folder?" -i "//media/daavvis/storage/uploadfolder/ota" FTPOTA
eval FTPOTA=$FTPOTA
	fi
